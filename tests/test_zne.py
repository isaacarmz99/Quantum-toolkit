...
from quantum_toolkit.mitigation.zne import fold_circuit, zero_noise_extrapolate
...
def test_zero_noise_extrapolation_beats_noisiest_raw_estimate():
    qc = _bell_state_without_measurement()
    backend = noisy_backend(depolarizing_prob_2q=0.12, readout_error_prob=0.05)

    result = zero_noise_extrapolate(
        qc, backend, target_bitstring="11", scale_factors=[1, 3, 5], shots=8192
    )

    ideal = 0.5
    noisiest_prob = result["probabilities"][-1]
    assert abs(result["zero_noise_estimate"] - ideal) < abs(noisiest_prob - ideal)